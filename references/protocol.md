...
Errors carry a bounded `reason` alongside their human-readable `error` and coarse `error_kind`. The reasons are `unknown`, `not_bound`, `binding_expired`, `not_authorized`, `extension_precommit_rejected`, `invalid_request`, `request_too_large`, `request_timeout`, `internal`, `version_mismatch`, `broker_unavailable`, `transport_lost`, and `malformed_response`. Missing, malformed, or unrecognized reasons become `unknown`; clients never recover a reason by matching prose. The broker/client/bridge/MCP path preserves reason metadata, and the OpenCode bridge preserves an internal broker error's kind. Framing failures remain general errors rather than newly definitive rejections.

`error_kind=rejected` describes the current request, not whether an earlier attempt of the same logical operation committed. Only that kind together with `reason=extension_precommit_rejected` releases a pending extension ID: the broker emits it at the phase and round-ceiling checks after checking the durable operation ID, before starting a new extension. Different-operation conflicts, authorization failures, recovery/commit errors, malformed success replies, and unknown reasons retain the ID. Both adapters validate the extension acknowledgement before clearing it; late completion or rejection cannot clear a replacement operation. Binding pruning is a separate policy and accepts only explicit `not_bound` or `binding_expired` rejections. Human prose is retained during this transition; legacy peers without reasons receive conservative handling.
