...
|---|---|---|
| Operating system | macOS only. Trust anchors use macOS code-signing evidence, process chains, and CDHash checks. | Full install/upgrade/rollback/uninstall lifecycle rehearsed clean-room on macOS 26.5.2 (Apple silicon), Standard (non-admin) account, from the public docs alone. Terminal-dialogue deletion and retention configuration exercised live 2026-08-25 on real dialogue state (delete → tombstone → idempotent duplicate; configure-retention → startup sweep → disable). |
...
|---|---|---|
| Codex (macOS app) | Session-authenticated MCP adapter (`council_mcp.py`) | Live triad dialogues + the maintainer's five-gate recovery matrix (router wake delivery, broker-restart barrier recovery, exactly-once redelivery, acknowledgement recovery, terminal delivery to all seats) passed 2026-08-25 on the v0.19.0 release tree, including three injected SIGKILL broker crashes mid-dialogue and a live router re-pairing after an app restart (see install.md, Codex wake router). Codex macOS host app 26.818.41509. Registration steps verified against a working install, not yet clean-room rehearsed. |
| Claude Code | Session-authenticated MCP adapter + live child relay | Same 2026-08-25 live matrix passed with Claude Code 2.1.241, including exactly-once redelivery to a fresh same-name bind after a host-recycled MCP adapter's seat aged out at lease expiry (the documented lifecycle interaction in [install.md](install.md)). The two-Claude-session quickstart ([quickstart.md](quickstart.md)) — registration, binds, and a full dialogue to `dialogue_complete` — was rehearsed clean-room 2026-08-24 by a non-author on a fresh macOS account from the public page alone: Claude Code 2.1.243, under 14 minutes including recovery from the multi-user socket-directory branch documented on that page (the `report` subcommand was not exercised in that run). |
| OpenCode CLI | Native custom tools + plugin-owned relay + offline executable pin | Setup rehearsed clean-room on OpenCode CLI 1.18.21: registration from docs alone, SDK auto-resolution (no npm needed), pin recorded (real path + SHA-256 + CDHash), post-pin restart with all eleven `council_*` tools registered. Same 2026-08-25 live matrix passed with 1.18.21, including exactly-once redelivery of a durably delivered, unanswered message across a SIGKILL broker crash (reauthentication-gated), and the OpenCode plugin restarting the crashed broker unprompted in all three crash recoveries. Pin must be manually renewed after every OpenCode upgrade; restart OpenCode after re-pinning. |
